...
hold('Applicative map', FC.asyncProperty(
  BracketArb(FC.string()),
  StringFunctionArb,
...
hold('ApplicativePar map', FC.asyncProperty(
  BracketArb(FC.string()),
  StringFunctionArb,
  (mx, f) => strErrEquivalence(
...
hold('Monadic map', FC.asyncProperty(
  BracketArb(FC.string()),
  StringFunctionArb,
  (mx, f) => strErrEquivalence(
...
hold('Monadic ap', FC.asyncProperty(
  BracketArb(FC.string()),
  BracketArb(StringFunctionArb),
...
hold('apFirst(ma)(mb) = apFirstPar(ma)(mb)', FC.asyncProperty(
  BracketArb(FC.string()),
  BracketArb(FC.string()),
  (mx, mb) => strErrEquivalence(
    Bracket.apFirst(mx)(mb),
    Bracket.apFirstPar(mx)(mb)
  )
));

hold('apSecond(ma)(mb) = apSecondPar(ma)(mb)', FC.asyncProperty(
  BracketArb(FC.string()),
  BracketArb(FC.string()),
  (mx, mb) => strErrEquivalence(
    Bracket.apSecond(mx)(mb),
    Bracket.apSecondPar(mx)(mb)
  )
));

hold('sequenceS(ms) = sequenceSPar(ms)', FC.asyncProperty(
  FC.dictionary(FC.string(), BracketArb(FC.string()), {minKeys: 1}),
  (ms) => recordErrEquivalence(
    Bracket.sequenceS(ms),
    Bracket.sequenceSPar(ms)
  )
));
